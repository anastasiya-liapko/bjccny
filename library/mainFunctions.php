...
/**
 * Преобразование результата работы функции выборки в ассоциативный массив с добавление тегов
 * 
 * @param recordset $rs набор строк - результат работы  SELECT
 * @param object $db база данных
 * @return array
 */
function createSmartyRsArrayWithTags($rs, $db) 
{
    if(! $rs) return false;
    ...
    while ($row = mysqli_fetch_assoc($rs)) {
        ...
        $rsPostTags = getPostTags($row['id'], $db);
